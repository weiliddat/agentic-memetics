...
- Point at `code-reference-formatting/SKILL.md` for code citations instead of restating reference rules. This applies to subagent reports as well as user-facing output. Exceptions: `critique` and `code-walkthrough` delegate citation formatting to the harness rather than depending on that skill.
