...
Search broadly for defects introduced or exposed by the changes, not just those that fit the emerging narrative or stated goal. Account for the full change set. Establish what affected users, callers, and downstream consumers rely on before the change, and what outcomes the change promises afterward. Ground those expectations in stated intent and existing usage. Trace whether the changed behavior still delivers them, including through unchanged code and alternative or failure paths; identify intentional changes separately from broken expectations. For intentional contract changes, assess compatibility and the transition, including for consumers that cannot update together. Use focused checks or reproductions where useful; do not equate a clean test run with complete coverage or claim that every possible issue has been excluded.

Examine affected trust boundaries and operating assumptions: who can act on what, which inputs are trusted, and how resource use and behavior change with load, data size, or concurrency. Investigate security, access-control, and performance consequences where the changes could introduce or expose them, grounding concerns in concrete paths and relevant operating conditions.

When a change affects state across requests, processes, or repeated runs, judge the whole operation by its intended outcome. Trace a meaningful interruption or concurrent change, then follow what the next attempt and other readers observe. A successful write may leave dependent state stale; a retry that performs no writes may still owe work. Verify that recovery restores the intended outcome, rather than merely avoiding duplicate execution.
