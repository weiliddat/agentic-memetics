...
Search broadly for defects introduced or exposed by the changes, not just those that fit the emerging narrative or stated goal. Account for the full change set. Compare old and new behavior, tracing affected contracts and assumptions through callers, state changes, and consumers—including unchanged code, external services, and users. When a contract changes, establish who relies on the previous behavior and whether compatibility or the transition is adequately addressed, including for consumers that cannot update together. Examine relevant alternative and failure paths; an intentional break still needs a supported rationale and a workable transition where required. Use focused checks or reproductions where useful; do not equate a clean test run with complete coverage or claim that every possible issue has been excluded.
